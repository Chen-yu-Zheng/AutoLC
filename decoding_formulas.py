...
class Decoder(object):
    def __init__(self, alphas, betas, steps):
        ...
        self._alphas = alphas
        self._steps = steps
        self._num_layers = self.betas.shape[0]
        self.network_space = torch.zeros(self._num_layers, 4, 3)

        for layer in range(self._num_layers):
            if layer == 0:
                self.network_space[layer][0][1] = F.softmax(self.betas[layer][0][1], dim=-1)
                self.network_space[layer][1][0] = F.softmax(self.betas[layer][1][0], dim=-1)
            elif layer == 1:
                self.network_space[layer][0][1:] = F.softmax(self.betas[layer][0][1:], dim=-1)
                self.network_space[layer][1][:2] = F.softmax(self.betas[layer][1][:2], dim=-1)
                self.network_space[layer][2][0] = F.softmax(self.betas[layer][2][0], dim=-1)

            elif layer == 2:
                self.network_space[layer][0][1:] = F.softmax(self.betas[layer][0][1:], dim=-1)
                self.network_space[layer][1] = F.softmax(self.betas[layer][1], dim=-1)
                self.network_space[layer][2][:2] = F.softmax(self.betas[layer][2][:2], dim=-1)
                self.network_space[layer][3][0] = F.softmax(self.betas[layer][3][0], dim=-1)

            else:
                self.network_space[layer][0][1:] = F.softmax(self.betas[layer][0][1:], dim=-1)
                self.network_space[layer][1] = F.softmax(self.betas[layer][1], dim=-1)
                self.network_space[layer][2] = F.softmax(self.betas[layer][2], dim=-1)
                self.network_space[layer][3][:2] = F.softmax(self.betas[layer][3][:2], dim=-1)
    ...
    def viterbi_decode(self):
        ...
        for layer in range(self.network_space.shape[0]):
            if layer == 0:
                ...
                prob_space[layer][1] = self.network_space[layer][1][0]
                ...
                for sample in range(self.network_space.shape[1]):
                    if layer - sample < - 1:
                        continue
                    local_prob = [0] * 3
                    for rate in range(self.network_space.shape[2]):  # rate[0 : ➘, 1: ➙, 2 : ➚]
                        if (sample == 0 and rate == 0) or (sample == 3 and rate == 2):
                            continue
                        else:
                            local_prob[rate] = (prob_space[layer - 1][sample + rate - 1] * self.network_space[layer][sample][rate])
