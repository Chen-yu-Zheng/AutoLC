...
class Cell(nn.Module):
    ...
    def scale_dimension(self, dim, scale):
        ...
        #STILL ERROR
        # return int((float(dim) - 1.0) * scale + 1.0) if (dim % 2 and scale < 1) else int(dim * scale)
    ...
    def forward(self, s0, s1_down, s1_same, s1_up, n_alphas):
        ...
        #prepre + (pre_up or pre_down or pre_up)
        ...
        for states in all_states:#down,same,up,3种连续的情况
            offset = 0
            for i in range(self._steps):
                new_states = []
            ...
            final_concates.append(concat_feature) #3种情况对应的输出，应该是按beta加权相加的
